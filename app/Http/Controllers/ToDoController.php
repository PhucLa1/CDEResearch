...
class ToDoController extends Controller
{
        ...
        try {
            ...
            if ($todo_permission == 0 || User::returnRole($project_id) == 1) {
                $todos = ToDo::leftJoin('users', 'todo.assgin_to', '=', 'users.email')
                    ...
                    ->select('todo.*','users.id as user_id');
                ...
                if (count($users) != 0) {
                    $todos = $todos->whereIn('users.id', $users)->orWhereIn('todo.user_create', $users);
                }
                if (count($status) != 0) {
                ...
                return response()->json([
                    'metadata' => $todos->whereBetween('todo.created_at', [$start_date, $finish_date])->with('user_create')->orderBy('created_at', 'desc')->get(),
            ...
            $todos = ToDo::leftJoin('users', 'todo.assgin_to', '=', 'users.email')
                ...
                ->orWhere('todo.user_create', $loguser)
                ->select('todo.*'); //Người tạo
            if (in_array(1, $owns)) { //Người được giao nhiệm vụ
                //$todos = $todos->where('users.id',$loguser);
            }
            if (in_array(2, $owns)) {
    ...
    public function store(Request $request)
    {
        ...
        $dataReturn = Todo::latest()->with('user')->with('user_create')->with('file.user')->first();
